...
function SummaryResultContent() {
  ...
  return (
    <main className="relative mx-auto min-h-dvh w-full max-w-[480px] bg-[#F7F3E7]">
      <p className={`${inter.className} px-6 pt-8 text-xs font-medium tracking-widest text-[#757057]`}>LIRA/SUMMARY</p>
      ...
      <div className="animate-fade-in-up translate-y-6 px-6 opacity-0">
        ...
        <section className="mx-auto flex aspect-[3/4] w-[76%] flex-col items-center justify-center rounded-[1.5rem] bg-[#8EF7D9] p-6 text-center">
          {loading ? <p className="text-base text-black/[0.48]">加载中...</p> : null}
          {error ? <p className="text-base text-[#A24C47]">{error}</p> : null}
          {!loading && !error ? (
            <p className="text-lg font-normal leading-relaxed text-black/[0.48]">{result?.chatSummary || '暂无总结'}</p>
          ) : null}
        </section>
      </div>
      ...
      <div className="absolute bottom-12 left-0 right-0 w-full flex justify-center">
        <button
          type="button"
          onClick={() => {
            router.push(`/end-chat/reflection?sessionId=${encodeURIComponent(sessionId)}`);
          }}
